...
import { StatementSubscription } from '../../src/types/subscription/StatementSubscription';
import validateStatementSubscription, {
	getStatementSubscriptionId,
} from '../../src/types/subscription/ValidateStatementSubscription';
...
export async function setAdminsToNewStatement(ev: any) {
	...
	try {
		...
		validateStatementSubscription(newSubscription);
		...
		adminsSubscriptions.forEach(async (adminSub: StatementSubscription) => {
			try {
				...
				const result = validateStatementSubscription(newSubscription);
				if (!result) {
					logger.warn(result);
