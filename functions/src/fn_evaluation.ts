...
function calcAgreement(
	...
	try {
		if (
			validate<number>(newSumEvaluations, ValidationType.Number) &&
			validate<number>(numberOfEvaluators, ValidationType.Number)
		)
			throw new Error(
				'newSumEvaluations or numberOfEvaluators is not a number'
			);
...
async function _updateStatementEvaluation({
	...
	try {
		...
		const { success } = safeParse(evaluationDiff, ValidationType.Number);
		if (!success)
			throw new Error('evaluation is not a number, or evaluation is missing');
...
export function updateChosenOptions(
